  ...
  <main class="justify-center sm:mx-auto sm:w-full sm:max-w-md">
    ...
    <div class="card w-full mt-4 shadow-2xl bg-base-200">
      ...
      <div class="card-body pt-0">
        <template v-if="refInfo && refInfo.name">
          <p class="text-center text-primary-content text-md mb-1"><span class="font-bold">{{ refInfo.name }}</span>
            invited you to join us ! <br> You'll receive <span class="inline-flex"> <coin-svg height="12px"
                                                                                              width="12px"/></span>
            <span class="text-base font-bold">
              {{ refInfo.points | formatNumber }}
            </span> if you join with us now.
          </p>
        </template>
        <template v-else>
...
export default {
  ...
  created() {
    ...
    this.$axios.get('/ref-info', {params: {username: this.$route.params.username}})
        .then(({data}) => (this.refInfo = data))
        .catch((error) => {
  ...
  methods: {
    register() {
      this.form.referrer_id = this.$route.params.username;
      this.loading = true;
      this.errors = {};
